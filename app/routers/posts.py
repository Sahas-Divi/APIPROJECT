...
def get_all_posts(db:Session=Depends(get_db),current_user: int = Depends(oath2.get_current_user),limit : int = 10,
skip:int = 0, search:Optional[str]=""):
    ...
    results = db.query(models.Post, func.count(models.Vote.posts_id).label("votes")).join(
        models.Vote, models.Vote.posts_id == models.Post.id, isouter = True).group_by(models.Post.id).filter(
            models.Post.title.contains(search)).limit(limit).offset(skip).all()
    return results


@router.get("/myposts", response_model = List[schemas.PostOut])
def get_myposts(db: Session = Depends(get_db), current_user = Depends(oath2.get_current_user)):
    myposts = db.query(models.Post, func.count(models.Vote.posts_id).label("votes")).join(
        models.Vote, models.Post.id == models.Vote.posts_id ,isouter=True).group_by(models.Post.id).filter(
...
def get_post(id:int, db: Session = Depends(get_db),current_user : int = Depends(oath2.get_current_user)):
    #cursor.execute("SELECT * FROM posts  WHERE id = {}".format(str(id)))
